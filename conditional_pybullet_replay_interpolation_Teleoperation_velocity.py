...
from class_poolingmodel_2nd_update_velocity import PoolingModel
...
class TeleopUnconditionalExecutor:
    def __init__(self, model_path: str,
                 collection_duration: float = 5.0,  # Teleoperation duration
    ...
    def inference_callback(self, _event):
        ...
        if self.model_prediction_active and not self.prediction_done:
            ...
            for i in range(init_len - 1, total_len - 1):
                ...
                self.mu = mu
                reference[i + 1] =reference[i]+ self.delta_t* self.mu[i]
                print(mu)
